...
class User (BaseHandler):
    ...
    async def start(start : types.Message, state : FSMContext):
        ...
        elif userData['offerta'] == False:
            await User._get_offerta(start, state)
        elif userData['tfone'] == None or userData['tfone'] == '' or  userData['tfone'] == 'None':
    ...
    async def start_callback(call : types.CallbackQuery, state : FSMContext):
        ...
        if userData['offerta'] == False:
            await User._get_offerta(call, state)
        elif userData['tfone'] == None:
    ...
    async def seller(seller : types.CallbackQuery|types.Message, state : FSMContext):
        from handlers.seller.location.location_class import Location      
        ...
        from handlers.seller.location.name import Name
        await state.clear()
        if_location = await Location.if_location(seller.from_user.id)
        if_contact = await Contacts.if_contacts(seller.from_user.id)
        if_organization = await Organization.if_organization(seller.from_user.id)
        if not if_location:
            ...
            await Organization().start_of_step(seller, state)
            return
        else:
            keyboard = await User.__get_seller_menu()
            await User.message_answer(seller, "Вы вошли как Продавец!", keyboard)
            return

    @staticmethod
    async def buyer(call : types.CallbackQuery|types.Message, state : FSMContext):
        await state.clear()
        userData = await User._search_user(call.from_user.id)
        if not userData['city']:
            ...
            await User.message_answer(call, "Вы вошли как Покупатель!", buyer_keyboard)

    @staticmethod
    async def loader(call : types.CallbackQuery|types.Message, state : FSMContext):
        from handlers.loader.location import Location, Name
        await state.clear()
        if_location = await Location.if_location(call.from_user.id)
        if not if_location:
            await Name().start_of_step(call, state)
            return
        else:
            keyboard = await User.__get_loader_menu()
            await User.message_answer(call, "Вы вошли как Грузчик!", keyboard)
            return
    ...
    async def profile(call : types.CallbackQuery, state : FSMContext):
        ...
        text_profile = await User.__get_text_profile(userData, contacts_all, Location_data)   
        inline_builder = await User.__get_inline_keyboard_profile()        
        await User.message_answer(call, text_profile, inline_builder)
    ...
    async def get_city_states(call : types.CallbackQuery, state : FSMContext):
        ...
        await User.message_answer(call, "Из какого вы города?")
    ...
    async def set_user_city(message : types.Message, state : FSMContext):
        ...
        await User.message_answer(message, "Вы из города " + state_data["city"] + "?", builder.as_markup())
    ...
    async def about_bot(call : types.CallbackQuery):
        ...
        builder.row(types.InlineKeyboardButton(text="🚚 Грузчик", callback_data="loader"))
        builder.row(types.InlineKeyboardButton(text="🏠 В главное меню", callback_data="main_menu"))
        await User.message_answer(call, text, builder.as_markup())
    ...
    async def _if_subscribshed(user_id : int):
        ...
        data_user_date = datetime.min if data_user['date_end_subscription'] is None else datetime.combine(data_user['date_end_subscription'], datetime.min.time())
        return (bool(data_user['subscription']) and (data_user_date > cur_date))
    @staticmethod
    ...
    async def _get_t_phone(start : types.Message|types.CallbackQuery, state : FSMContext):
        ...
        await User.message_answer(start, text, markup)
    ...
    async def __get_text_organization(userData):
        return f"Организация: {userData['organization']}"
    
    @staticmethod
    async def __get_text_location(location_data):
        text = ''
        text += f'корпус: {location_data["name_of_place"]} ' if location_data["name_of_place"]  else ''
        text += f'сектор: {location_data["sector"]}' if location_data['sector'] else ''
        text += f'здание: {location_data["building"]} ' if location_data['building'] else ''
        text += f'этаж: {location_data["floar"]} ' if location_data['floar'] else ''
        text += f'ряд: {location_data["line"]} ' if location_data['line'] else ''
        text += f'место: {location_data["place"]} ' if location_data['place'] else ''
        text += f'адрес: {location_data["address"]} ' if location_data['address'] else ''
