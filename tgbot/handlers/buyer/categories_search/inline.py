...
class Inline (StepsBase):
        ...
        for categorie in categories:
            results.append(
                InlineQueryResultArticle(
                    id=str(categorie['hash']),
                    title=categorie['category_three'],
                    input_message_content=InputTextMessageContent(
                        message_text=f"Вы выбрали категорию: {categorie['category_one'] + '->' + categorie['category_two'] + '->' + categorie['category_three']}", parse_mode="HTML"
                    ),
                    description=f"{categorie['category_one'] + '->' + categorie['category_two'] + '->' + categorie['category_three']}",
                )
            )
        if len(results) == 0:
            results.append(
    ...
    async def _process_message(self, message: types.Message, state: FSMContext):
        ...
        if match:
            data_state['categories_search_category_one_level'], data_state['categories_search_category_two_level'], data_state['categories_search_category_three_level'] = [match.group(1).strip().lower(), match.group(2).strip().lower(), match.group(3).strip().lower()]
    ...
    async def _go_to_next_step(self, message: types.Message, state: FSMContext): 
        from . import Confirm
        await Confirm().start_of_step(message, state)
