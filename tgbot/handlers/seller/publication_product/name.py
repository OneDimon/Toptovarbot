...
from database.seller.product import ProductDatabase as DB_product
...
class Name (StepsBase):
    ...
    async def _if_option_publish(self, call: types.CallbackQuery, state: FSMContext) -> bool:
        ...
        if not await User._if_subscribshed(call.from_user.id):
            ...
            if product:
                await self.message_answer(call, 'Вы уже публиковали свой товар сегодня! Оплатите подписку или приходите завтра.')
    ...
    async def _initialize_publication_product_data_state(self, call: types.CallbackQuery, state: FSMContext):
        from database.system.categories_product import CategoriesProductDatabase
