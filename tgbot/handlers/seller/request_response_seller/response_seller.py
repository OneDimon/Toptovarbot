from database.seller.request_response_seller.request_response_seller import RequestResponseSellerDatabase as DB
import aiogram
...
class ResponseSeller():
    ...
    async def send_request_info(self, call: types.CallbackQuery, state: FSMContext):
        ...
        str_response_two = (f"категория :{request_info['categoy_all']['category_one']}->{request_info['categoy_all']['category_two']}->{request_info['categoy_all']['category_three']} названи е :{request_info['product']} "
                          f"фото")
